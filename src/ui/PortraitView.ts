...
export function createConceptPortrait(
  ...
  if (scene.textures.exists(asset.key)) {
    ...
    const innerWidth = Math.max(1, Math.round(width) - 8);
    const innerHeight = Math.max(1, Math.round(height) - 8);
    ...
    if (scene.textures.exists(missingKey)) {
      ...
      const scale = Math.min((width - 8) / missing.width, (height - 8) / missing.height);
      children.push(missing.setScale(scale));
    } else {
      children.push(pixelText(scene, 0, 0, 'ASSET\nAUSENTE', { size: 12, color: PALETTE.danger, align: 'center' }));
    }
    console.error(`[Rua de Aço] Retrato conceitual ausente no cache: ${asset.key}`);
  }

  if (options.locked) {
    children.push(scene.add.rectangle(0, 0, Math.round(width) - 4, Math.round(height) - 4, PALETTE.black, 0.58));
    children.push(pixelText(scene, 0, 0, 'EM DEV', { size: 12, color: PALETTE.gold, align: 'center' }));
  }
