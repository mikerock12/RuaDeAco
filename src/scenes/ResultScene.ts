...
import { drawRemasterBackdrop, UI_THEME, focusPanel } from '../ui/remasterTheme';
...
export class ResultScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.rectangle(130, 72, 188, 1, PALETTE.gold).setDepth(6);
  ...
  private drawBackground(): void {
    ...
    this.add.rectangle(320, 180, 608, 336, UI_THEME.panel, 0.68).setStrokeStyle(1, UI_THEME.border);
    this.add.rectangle(320, 188, 1, 284, UI_THEME.border);
  }
  ...
  private createOption(x: number, y: number, text: string, activate: () => void): ResultOption {
    ...
    const background = tagLayoutPanel(
      this.add.rectangle(0, 0, 288, 40, UI_THEME.panel, 0.86)
  ...
  private refreshSelection(): void {
    this.options.forEach((option, index) => {
      ...
      focusPanel(option.background, selected, true);
