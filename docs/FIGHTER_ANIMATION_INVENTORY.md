# Inventário de Animações dos Lutadores

Este documento mapeia o status dos sprites de luta e especiais criados a partir dos cards de referência.

## Legenda de Status
- ❌ **Ausente**: Nenhum arquivo existe.
- ⚠️ **Placeholder ruim**: Arquivo genérico/duplicado sem relação com o card.
- ⚙️ **Em produção**: Arquivo isolado do card, em processo de ajustes de spritesheet.
- ✅ **Concluído**: Spritesheet com canvas padronizado por lutador, gerado corretamente e com animação própria.
- 🚀 **Validado no jogo**: Spritesheet conectado, configurado e testado no jogo (hitboxes, frameRate).

---

## Rafa Maré (Agile / Rushdown)
- **Idle**: ✅ Concluído
- **Corrida**: ✅ Concluído
- **Mão da Maré**: ✅ Concluído
- **Chute da Ressaca**: ✅ Concluído
- **Eco Tatuado**: ✅ Concluído

## Guto Barba (Tank / Grappler)
- **Idle**: ✅ Concluído
- **Corrida**: ✅ Concluído
- **Muralha Norte**: ✅ Concluído
- **Gancho do Urso**: ✅ Concluído
- **Abraço Glacial**: ✅ Concluído

## Noir Reflexo (Counter / Zoner)
- **Idle**: ✅ Concluído
- **Corrida**: ✅ Concluído
- **Reflexo Negro**: ✅ Concluído
- **Quebra-Luz**: ✅ Concluído
- **Impacto Solar**: ✅ Concluído

## Dante Sinal (Technical / Trapper)
- **Idle**: ✅ Concluído
- **Corrida**: ✅ Concluído
- **Ponto Final**: ✅ Concluído
- **Cortina Óptica**: ✅ Concluído
- **Chave Binária**: ✅ Concluído

## Léo Violeta (Pressure / Brawler)
- **Idle**: ✅ Concluído
- **Corrida**: ✅ Concluído
- **Olhar Frio**: ✅ Concluído
- **Impacto Sombrio**: ✅ Concluído
- **Pressão Violeta**: ✅ Concluído

## Astro Riso (Speed / Mix-up)
- **Pacote corporal compartilhado (35 folhas)**: 🚀 Validado no jogo
- **Idle**: 🚀 Validado no jogo
- **Corrida**: 🚀 Validado no jogo
- **Sorriso Relâmpago + efeito separado**: 🚀 Validado no jogo
- **Rajada Neon + efeito separado**: 🚀 Validado no jogo
- **Astro Giro + efeito separado**: 🚀 Validado no jogo

---
*(Estes status representam a extração automatizada de assets perfeitos diretamente das referências, evitando placeholders).*
